 ...
 */
@Service
public class SerializeSegmentServiceImpl implements SerializeSegmentService {
	...
	@Override
	public SerializableSection serializeSegment(SegmentLink segmentLink, String prefix, Integer position,
			Integer headerLevel, UsageConfig segmentUsageConfig, Boolean greyOutOBX2FlavorColumn,
			IncludeAutoGenerated includeAuto, CoConstraintExportMode coConstraintExportMode,
			Boolean includeDynamicMapping, BindingExportConfig bindingExportConfig)
			throws SegmentSerializationException {
		Segment segment = segmentService.findById(segmentLink.getId());
		return this.serializeSegment(segment, prefix, position, headerLevel, segmentUsageConfig, null, null, false,
				greyOutOBX2FlavorColumn, null, includeAuto, coConstraintExportMode, includeDynamicMapping,
				bindingExportConfig);
	}

	@Override
	public SerializableSection serializeSegment(SegmentLink segmentLink, String prefix, Integer position,
			Integer headerLevel, UsageConfig segmentUsageConfig, Map<String, Segment> compositeProfileSegments,
			Map<String, Datatype> compositeProfileDatatypes, Map<String, Table> compositeProfileTables,
			Boolean greyOutOBX2FlavorColumn, IncludeAutoGenerated includeAuto,
		...
		if (segment.isTemporary() && !includeAuto.segment) {
			return null;
		}
		return this.serializeSegment(segment, prefix, position, headerLevel, segmentUsageConfig,
				compositeProfileDatatypes, compositeProfileTables, false, greyOutOBX2FlavorColumn, null, includeAuto,
				coConstraintExportMode, includeDynamicMapping, bindingExportConfig);
	}

	private SerializableSection serializeSegment(Segment segment, String prefix, Integer position, Integer headerLevel,
			UsageConfig fieldUsageConfig, Map<String, Datatype> compositeProfileDatatypes,
			Map<String, Table> compositeProfileTables, Boolean showInnerLinks, Boolean greyOutOBX2FlavorColumn,
			String host, IncludeAutoGenerated includeAuto, CoConstraintExportMode coConstraintExportMode,
			Boolean includeDynamicMapping, BindingExportConfig bindingExportConfig)
			throws SegmentSerializationException {
		if (segment != null) {
			try {
				...
				SerializableSection serializableSegmentSection = new SerializableSection(id, prefix, segmentPosition,
						sectionHeaderLevel, title);
				// create segment node
				id = segment.getId();
				String name = segment.getName();
				String label = segment.getExt() == null || segment.getExt().isEmpty() ? segment.getName()
						: segment.getLabel();
				segmentPosition = "";
				...
				List<SerializableConstraint> constraints = serializeConstraintService.serializeConstraints(segment,
						segment.getName() + "-");
				...
				for (ValueSetOrSingleCodeBinding valueSetOrSingleCodeBinding : segment.getValueSetBindings()) {
					if (valueSetOrSingleCodeBinding.getTableId() != null
						...
						if (ExportUtil.diplayUsage(field.getUsage(), fieldUsageConfig)) {
							if (field.getDatatype() != null) {
								...
								if (compositeProfileDatatypes != null && !compositeProfileDatatypes.isEmpty()) {
									datatype = findDatatypeInCompositeProfileDatatypes(field.getDatatype().getId(),
											compositeProfileDatatypes);
								}
								if (datatype == null) {
							...
							for (ValueSetOrSingleCodeBinding valueSetOrSingleCodeBinding : segment
									.getValueSetBindings()) {
								if (serializationUtil.getParentLocation(valueSetOrSingleCodeBinding.getLocation())
										.equals(String.valueOf(field.getPosition()))) {
				...
				if (segment.getCoConstraintsTable() != null && segment.getCoConstraintsTable().getRowSize() > 0) {
					...
					for (int i = 0; i < segment.getCoConstraintsTable().getRowSize(); i++) {
						for (CoConstraintColumnDefinition coConstraintColumnDefinition : segment.getCoConstraintsTable()
								.getThenColumnDefinitionList()) {
							CoConstraintTHENColumnData coConstraintTHENColumnData = segment.getCoConstraintsTable()
						...
						for (DynamicMappingItem dynamicMappingItem : dynamicMappingDefinition
							...
							try {
								...
								throw new DynamicMappingItemSerializationException(e,
										dynamicMappingDefinition.getDynamicMappingItems().indexOf(dynamicMappingItem)
												+ 1);
							}
						}
					} catch (Exception e) {
				...
				SerializableSegment serializableSegment = new SerializableSegment(id, prefix, segmentPosition,
						sectionHeaderLevel, title, segment, name, label, description, comment, defPreText, defPostText,
						constraints, fieldDatatypeMap, fieldValueSetBindingsMap, tables, coConstraintValueTableMap,
						dynamicMappingDatatypeMap, showConfLength, showInnerLinks, greyOutOBX2FlavorColumn, host,
						coConstraintDatatypeMap, includeAuto, coConstraintExportMode, filtredBinding,
	...
	public SerializableSection serializeSegment(Segment segment, String host) throws SegmentSerializationException {
		...
		return serializeSegment(segment, String.valueOf(0), 1, 1, defaultConfig.getFieldsExport(), null, null, true,
				false, host, defaultConfig.getIncludeAutoGenerated(), defaultConfig.getCoConstraintExportMode(), true,
				new BindingExportConfig());
	}
