...
import gov.nist.healthcare.tools.hl7.v2.igamt.lite.domain.IncludeAutoGenerated;
...
public abstract class SerializeMessageOrCompositeProfile {
	...
	protected void serializeSegment(SegmentRefOrGroup segmentRefOrGroup, String prefix,
			SerializableSection segmentsSection, UsageConfig segmentRefOrGroupUsageConfig,
			UsageConfig segmentUsageConfig, UsageConfig fieldsUsageConfig, Boolean duplicateOBXDataTypeWhenFlavorNull,
			IncludeAutoGenerated includeAuto, CoConstraintExportMode coConstraintExportMode)
			throws SegmentSerializationException {
		serializeSegment(segmentRefOrGroup, prefix, segmentsSection, segmentRefOrGroupUsageConfig, segmentUsageConfig,
				fieldsUsageConfig, duplicateOBXDataTypeWhenFlavorNull, null, coConstraintExportMode);
	}

	protected void serializeSegment(SegmentRefOrGroup segmentRefOrGroup, String prefix,
			SerializableSection segmentsSection, UsageConfig segmentRefOrGroupUsageConfig,
			UsageConfig segmentUsageConfig, UsageConfig fieldsUsageConfig, Boolean duplicateOBXDataTypeWhenFlavorNull,
			Map<String, Segment> compositeProfileSegments, IncludeAutoGenerated includeAuto,
			CoConstraintExportMode coConstraintExportMode, Boolean includeDynamicMapping,
			BindingExportConfig bindingExportConfig) throws SegmentSerializationException {
			...
			if (segmentRefOrGroup instanceof SegmentRef) {
				...
				if (!messageSegmentsNameList.contains(segmentLink.getId())) {
					segmentsSection.addSection(serializeSegmentService.serializeSegment(segmentLink,
							prefix + String.valueOf(segmentPosition), segmentPosition, 5, fieldsUsageConfig,
							duplicateOBXDataTypeWhenFlavorNull, includeAuto, coConstraintExportMode,
							includeDynamicMapping, bindingExportConfig));
					messageSegmentsNameList.add(segmentLink.getId());
					segmentPosition++;
				}
			} else if (segmentRefOrGroup instanceof Group) {
				/*
				 * String id = UUID.randomUUID().toString(); String headerLevel
				 * = String.valueOf(4); String title = ((Group)
				 * segmentRefOrGroup).getName(); SerializableSection
				 * serializableSection = new
				 * SerializableSection(id,prefix,String.valueOf(position),
				 * headerLevel,title);
				 */
				for (SegmentRefOrGroup groupSegmentRefOrGroup : ((Group) segmentRefOrGroup).getChildren()) {
					serializeSegment(groupSegmentRefOrGroup, prefix, segmentsSection, segmentRefOrGroupUsageConfig,
							segmentUsageConfig, fieldsUsageConfig, duplicateOBXDataTypeWhenFlavorNull, includeAuto,
	...
	protected SerializableConstraints serializeConstraints(List<? extends Constraint> constraints, String name,
			int position, String type) throws ConstraintSerializationException {
		...
		SerializableConstraints serializableConstraints = new SerializableConstraints(serializableConstraintList, id,
				String.valueOf(position), name, type);
		return serializableConstraints;

	}

	protected SerializableSegmentRefOrGroup serializeSegmentRefOrGroup(SegmentRefOrGroup segmentRefOrGroup,
			UsageConfig segmentUsageConfig, UsageConfig fieldUsageConfig, Map<String, Segment> compositeProfileSegments)
			throws SerializationException {
		return this.serializeSegmentRefOrGroup(segmentRefOrGroup, segmentUsageConfig, fieldUsageConfig,
				compositeProfileSegments, false, null);
	}

	protected SerializableSegmentRefOrGroup serializeSegmentRefOrGroup(SegmentRefOrGroup segmentRefOrGroup,
			UsageConfig segmentUsageConfig, UsageConfig fieldUsageConfig, Map<String, Segment> compositeProfileSegments,
			Boolean showInnerLinks, String host) throws SerializationException {
	...
	private SerializableSegmentRefOrGroup serializeSegmentRef(SegmentRef segmentRef, UsageConfig usageConfig,
			Map<String, Segment> compositeProfileSegments, Boolean showInnerLinks, String host)
			throws SegmentSerializationException {
		...
		if (segmentLink != null) {
			try {
				...
				serializableSegmentRefOrGroup = new SerializableSegmentRefOrGroup(segmentRef, segment,
						this instanceof SerializeCompositeProfileService, showInnerLinks, host);
	...
	private SerializableSegmentRefOrGroup serializeGroup(Group group, UsageConfig segmentUsageConfig,
			UsageConfig fieldUsageConfig, Map<String, Segment> compositeProfileSegments, Boolean showInnerLinks,
			String host) throws SerializationException {
		try {
			...
			List<SerializableConstraint> groupConstraints = serializeConstraintService.serializeConstraints(group,
					group.getName());
			serializableGroup = new SerializableSegmentRefOrGroup(group, serializableSegmentRefOrGroups,
	...
	protected HashMap<String, String> retrieveComponentsPaths(Message message) {
		...
		for (Comment comment : message.getComments()) {
			if (comment != null && comment.getLocation() != null && !comment.getLocation().isEmpty()
					&& !locationsToRetrieve.contains(comment.getLocation())) {
		...
		for (ValueSetOrSingleCodeBinding valueSetOrSingleCodeBinding : message.getValueSetBindings()) {
			if (valueSetOrSingleCodeBinding != null && valueSetOrSingleCodeBinding.getLocation() != null
					&& !valueSetOrSingleCodeBinding.getLocation().isEmpty()
					&& !locationsToRetrieve.contains(valueSetOrSingleCodeBinding.getLocation())) {
	...
	protected HashMap<String, String> retrieveComponentsPaths(CompositeProfile compositeProfile) {
		...
		return retrieveComponentsPaths(locationsToRetrieve, compositeProfile.getChildren(),
				compositeProfile.getSegmentsMap());
	}

	private HashMap<String, String> retrieveComponentsPaths(HashSet<String> locationsToRetrieve,
			List<SegmentRefOrGroup> segmentRefOrGroups) {
		return retrieveComponentsPaths(locationsToRetrieve, segmentRefOrGroups, null);
	}

	private HashMap<String, String> retrieveComponentsPaths(HashSet<String> locationsToRetrieve,
			List<SegmentRefOrGroup> segmentRefOrGroups, Map<String, Segment> compositeProfileSegmentsMap) {
				...
				try {
					...
					for (SegmentRefOrGroup segmentRefOrGroup : segmentRefOrGroups) {
						if (segmentRefOrGroup.getPosition() == locationToken) {
							path = this.retrieveSegmentOrGroupName(segmentRefOrGroup, stringTokenizer,
									compositeProfileSegmentsMap);
							break;
						}
					}
	...
	private String retrieveSegmentOrGroupName(SegmentRefOrGroup segmentRefOrGroup, StringTokenizer stringTokenizer,
			Map<String, Segment> compositeProfileSegmentsMap) {
		if (segmentRefOrGroup instanceof SegmentRef) {
			Segment segment = null;
				...
				try {
					...
					for (SegmentRefOrGroup groupSegmentRefOrGroup : group.getChildren()) {
						if (groupSegmentRefOrGroup.getPosition() == location) {
							return group.getName() + "." + retrieveSegmentOrGroupName(groupSegmentRefOrGroup,
									stringTokenizer, compositeProfileSegmentsMap);
						}
					}
				} catch (NumberFormatException nfe) {
					logger.error(
							"Unable to retreive path: Comment group's segment location is malformed [" + token + "]");
				}
			}
