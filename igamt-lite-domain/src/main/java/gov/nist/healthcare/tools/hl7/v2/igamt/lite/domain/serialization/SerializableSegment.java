...
public class SerializableSegment extends SerializableSection {
	...
	private IncludeAutoGenerated includeAuto;
	private Boolean includeDynamicMapping;
	...
	public SerializableSegment(String id, String prefix, String position, String headerLevel, String title,
			Segment segment, String name, String label, String description, String comment, String defPreText,
			String defPostText, List<SerializableConstraint> constraints, Map<Field, Datatype> fieldDatatypeMap,
			...
			Map<String, Datatype> coConstraintDatatypeMap, IncludeAutoGenerated includeAuto,
			CoConstraintExportMode coConstraintExportMode, List<ValueSetOrSingleCodeBinding> filtredBindings,
			Boolean includeDynamicMapping, BindingExportConfig bindingConfig) {
		...
		this.includeDynamicMapping = includeDynamicMapping;
	}

	@Override
	public Element serializeElement() throws SegmentSerializationException {

		if (this.segment.isTemporary()) {
			return this.serializeAutoGenerated();
		}
		...
		try {
			if (segment != null) {
				...
				if (filtredBindings != null && !filtredBindings.isEmpty()) {
					if (this.bindingConfig != null && this.bindingConfig.isIncludeForSegment()) {
						Element valueSetBindingListElement = super.createValueSetBindingListElement(filtredBindings,
								this.tables, segment.getLabel(), this.bindingConfig);
				...
				if (segment.getComments() != null && !segment.getComments().isEmpty()) {
					Element commentListElement = super.createCommentListElement(segment.getComments(),
							segment.getLabel());
				...
				for (int i = 0; i < segment.getFields().size(); i++) {
					try {
						...
						if (this.fieldValueSetBindingsMap.containsKey(field)) {
							List<ValueSetOrSingleCodeBinding> valueSetBindings = this.fieldValueSetBindingsMap
									.get(field);
							if (valueSetBindings != null && !valueSetBindings.isEmpty()) {
								...
								for (ValueSetOrSingleCodeBinding valueSetOrSingleCodeBinding : valueSetBindings) {
									try {
										if (valueSetOrSingleCodeBinding != null
											...
											if (table != null) {
												...
												if (this.showInnerLinks && !"".equals(link)) {
													String wrappedLink = this.wrapLink(link,
										...
										throw new TableSerializationException(e,
												valueSetOrSingleCodeBinding.getLocation());
									}
								}
				...
				if (coConstraintsTable.getIfColumnData() != null && !coConstraintsTable.getIfColumnData().isEmpty()) {
					if (!segment.isTemporary() || coConstraintsTable.isTemporary() || includeAuto.coConstraints) {

						SerializableCoConstraints serializableCoConstraints = new SerializableCoConstraints(
								coConstraintsTable, this.segment.getName(), coConstraintValueTableMap,
				...
				if (dynamicMappingDefinition != null && !dynamicMappingDefinition.getDynamicMappingItems().isEmpty()) {
					if (this.includeDynamicMapping) {
						Element dynamicMappingElement = generateDynamicMappingElement(dynamicMappingDefinition,
	...
	private Element serializeAutoGenerated() throws SegmentSerializationException {
		Element segmentElement = new Element("Segment");
		Element segmentMetadata = super.createMetadataElement(segment);
		if (segmentMetadata != null) {
			segmentElement.appendChild(segmentMetadata);
		...
		try {
			if (segment != null) {
				...
				segmentElement.addAttribute(new Attribute("Description", this.description));
				segmentElement.addAttribute(new Attribute("ShowConfLength", String.valueOf(showConfLength)));
				if (this.includeAuto.comment) {
					if (this.comment != null && !this.comment.isEmpty()) {
						segmentElement.addAttribute(new Attribute("Comment", this.comment));
					}
				}
				if (this.includeAuto.preDef) {
					if ((segment.getText1() != null && !segment.getText1().isEmpty())
							|| (segment.getText2() != null && !segment.getText2().isEmpty())) {
						if (this.defPreText != null && !this.defPreText.isEmpty()) {
							segmentElement.appendChild(this.createTextElement("DefPreText", this.defPreText));
				...
				if (this.includeAuto.comments && segment.getTemps().comment) {
					if (segment.getComments() != null && !segment.getComments().isEmpty()) {
						Element commentListElement = super.createCommentListElement(segment.getComments(),
								segment.getLabel());
						if (commentListElement != null) {
				...
				for (int i = 0; i < segment.getFields().size(); i++) {
					try {
						...
						String comments = super.findComments(field.getPosition(), segment.getComments());
						// if (comments != null && !comments.isEmpty()){
						// if(field.getTemps().comment && includeAuto.comment)
						// fieldElement.addAttribute(new Attribute("Comment",
						// comments));
						// }
						fieldElement.addAttribute(new Attribute("Position", String.valueOf(field.getPosition())));
				...
				if (!constraints.isEmpty()) {
					for (SerializableConstraint constraint : constraints) {
						if (constraint.getConstraint() instanceof Predicate && this.includeAuto.predicates) {
							segmentElement.appendChild(constraint.serializeElement());
						}
						if (constraint.getConstraint() instanceof ConformanceStatement
								&& this.includeAuto.conformanceStatements) {
							segmentElement.appendChild(constraint.serializeElement());
						}
					}
				}
				CoConstraintsTable coConstraintsTable = segment.getCoConstraintsTable();
				if (coConstraintsTable.getIfColumnData() != null && !coConstraintsTable.getIfColumnData().isEmpty()) {
					if (coConstraintsTable.isTemporary() || includeAuto.coConstraints) {
						SerializableCoConstraints serializableCoConstraints = new SerializableCoConstraints(
								coConstraintsTable, this.segment.getName(), coConstraintValueTableMap,
								coConstraintDatatypeMap, coConstraintExportMode, greyOutOBX2FlavorColumn);
						Element coConstraintsElement = serializableCoConstraints.serializeElement();
				...
				DynamicMappingDefinition dynamicMappingDefinition = segment.getDynamicMappingDefinition();
				if (dynamicMappingDefinition != null && !dynamicMappingDefinition.getDynamicMappingItems().isEmpty()) {
					if (dynamicMappingDefinition.isTemporary() || includeAuto.dynamicMapping) {
						Element dynamicMappingElement = generateDynamicMappingElement(dynamicMappingDefinition,
								this.dynamicMappingDatatypeMap);
						if (dynamicMappingElement != null) {
							segmentElement.appendChild(dynamicMappingElement);
	...
	public static Element generateDynamicMappingElement(DynamicMappingDefinition dynamicMappingDefinition,
			Map<String, Datatype> dynamicMappingDatatypeMap) throws DynamicMappingSerializationException {
		try {
			...
			for (DynamicMappingItem dynamicMappingItem : dynamicMappingDefinition.getDynamicMappingItems()) {
				if (dynamicMappingItem != null) {
					...
					try {
						if (dynamicMappingItem.getFirstReferenceValue() != null
								&& !dynamicMappingItem.getFirstReferenceValue().isEmpty()) {
							dynamicMappingItemElement.addAttribute(
									new Attribute("FirstReferenceValue", dynamicMappingItem.getFirstReferenceValue()));
						} else {
							throw new DynamicMappingItemSerializationException(new Exception(),
									dynamicMappingDefinition.getDynamicMappingItems().indexOf(dynamicMappingItem) + 1,
									"Missing first reference value");
						}
						...
						/*
						 * Second reference value isn't required else { throw
						 * new DynamicMappingItemSerializationException(new
						 * Exception(), "DynamicMappingItem[" +
						 * dynamicMappingDefinition.getDynamicMappingItems()
						 * .indexOf(dynamicMappingItem) +
						 * "]","Missing second reference value"); }
						 */
						if (dynamicMappingItem.getDatatypeId() != null
								&& !dynamicMappingItem.getDatatypeId().isEmpty()) {
