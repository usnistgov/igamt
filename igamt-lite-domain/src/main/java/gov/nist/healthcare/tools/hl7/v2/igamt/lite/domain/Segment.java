...
public class Segment extends DataModelWithConstraints implements java.io.Serializable, Cloneable, Comparable<Segment> {
		...
		for (ValueSetOrSingleCodeBinding vsoscb : this.valueSetBindings) {
			if (vsoscb instanceof SingleCodeBinding) {
				...
				if (scb.isCodedElement()) {
					assertion = "<Assertion>" + "<AND>" + "<PlainText Path=\"" + path + ".1[1]" + "\" Text=\""
							+ scb.getCode().getValue() + "\" IgnoreCase=\"false\"/>" + "<PlainText Path=\"" + path
							+ ".3[1]" + "\" Text=\"" + scb.getCode().getCodeSystem() + "\" IgnoreCase=\"false\"/>"
							+ "</AND>" + "</Assertion>";
				} else {
					assertion = "<Assertion><PlainText Path=\"" + path + "\" Text=\"" + scb.getCode().getValue()
							+ "\" IgnoreCase=\"false\"/></Assertion>";
				}
				ConformanceStatement cs = new ConformanceStatement();
				cs.setId(ObjectId.get().toString());
								...
								if (isValidData(thenData, definitionThen, tablesMap)) {
									...
									if (isOBX5(definitionThen.getPath())) {
										if (thenData.getValueSets() != null && thenData.getValueSets().size() > 0) {
											...
											thenDescription = this.generateTHENDescirptionForValue(definitionThen,
													thenData);
											thenAssertion = this.generateTHENAssertionForValue(definitionThen,
													thenData);
										}
	...
	private String generateTHENAssertionForValue(CoConstraintColumnDefinition definitionThen,
		...
		if (definitionThen.isPrimitive()) {
			return "<OR><NOT><Presence Path=\"" + definitionThen.getConstraintPath() + "\"/></NOT><PlainText Path=\""
					+ definitionThen.getConstraintPath() + "\" Text=\"" + thenData.getValueData().getValue()
					+ "\" IgnoreCase=\"false\"/></OR>";
		} else {
			if (thenData.getValueData().getBindingLocation() == null) {
				return "<OR><NOT><Presence Path=\"" + definitionThen.getConstraintPath() + ".1[1]"
						+ "\"/></NOT><PlainText Path=\"" + definitionThen.getConstraintPath() + ".1[1]" + "\" Text=\""
						+ thenData.getValueData().getValue() + "\" IgnoreCase=\"false\"/></OR>";
			} else {
				// "1 or 4", "1 or 4 or 10"
				if (thenData.getValueData().getBindingLocation().equals("1 or 4")) {
					return "<OR>" + "<PlainText Path=\"" + definitionThen.getConstraintPath() + ".1[1]" + "\" Text=\""
							+ thenData.getValueData().getValue() + "\" IgnoreCase=\"false\"/>" + "<PlainText Path=\""
					...
					return "<OR><NOT><Presence Path=\"" + definitionThen.getConstraintPath() + "."
							+ thenData.getValueData().getBindingLocation() + "[1]" + "\"/></NOT><PlainText Path=\""
							+ definitionThen.getConstraintPath() + "." + thenData.getValueData().getBindingLocation()
							+ "[1]" + "\" Text=\"" + thenData.getValueData().getValue()
							+ "\" IgnoreCase=\"false\"/></OR>";
				}
			}
		}
	...
	private String generateTHENDescirptionForValueSet(CoConstraintColumnDefinition definitionThen,
		...
		String thenDescription = "then the value of " + this.getName() + "-" + definitionThen.getConstraintPath() + " ("
	...
	private String generateTHENAssertionForValueSet(CoConstraintColumnDefinition definitionThen,
		...
		if (thenData.getValueSets().size() == 1) {
			...
			if (definitionThen.isPrimitive()) {
				thenAssertion = "<ValueSet Path=\"" + definitionThen.getConstraintPath() + "\" ValueSetID=\"" + bid
			...
			for (ValueSetData vs : thenData.getValueSets()) {
				...
				thenAssertion = thenAssertion + "<ValueSet Path=\"" + definitionThen.getConstraintPath()
						+ "\" ValueSetID=\"" + bid + "\" BindingLocation=\"" + bindingLocation + "\" BindingStrength=\""
		...
		if (definitionThen.isPrimitive()) {
			if (isOBX5(definitionThen.getPath())) {
				thenAssertion = "<OR><NOT><Presence Path=\"" + definitionThen.getConstraintPath() + ".1[1]\"/></NOT>"
						+ thenAssertion + "</OR>";
			} else {
				thenAssertion = "<OR><NOT><Presence Path=\"" + definitionThen.getConstraintPath() + "\"/></NOT>"
						+ thenAssertion + "</OR>";
			}
		} else {
			if (thenData.getValueSets().get(0).getBindingLocation() == null) {
				...
				if (thenData.getValueSets().get(0).getBindingLocation().equals("1 or 4")) {
					thenAssertion = "<OR><AND><NOT><Presence Path=\"" + definitionThen.getConstraintPath() + ".1[1]"
							+ "\"/></NOT><NOT><Presence Path=\"" + definitionThen.getConstraintPath() + ".4[1]"
							+ "\"/></NOT></AND>" + thenAssertion + "</OR>";
				} else if (thenData.getValueSets().get(0).getBindingLocation().equals("1 or 4 or 10")) {
					thenAssertion = "<OR><AND><AND><NOT><Presence Path=\"" + definitionThen.getConstraintPath()
							+ ".1[1]" + "\"/></NOT><NOT><Presence Path=\"" + definitionThen.getConstraintPath()
							+ ".4[1]" + "\"/></NOT></AND><NOT><Presence Path=\"" + definitionThen.getConstraintPath()
							+ ".10[1]" + "\"/></NOT></AND>" + thenAssertion + "</OR>";
				} else {
					thenAssertion = "<OR<NOT><Presence Path=\"" + definitionThen.getConstraintPath() + "."
							+ thenData.getValueSets().get(0).getBindingLocation() + "[1]" + "\"/></NOT>" + thenAssertion
							+ "</OR>";
				}
			}
		}
	...
	private String generateIFAssertion(CoConstraintColumnDefinition definitionIF, CoConstraintIFColumnData ifData) {
		if (definitionIF.isPrimitive()) {
			...
			if (ifData.getValueData().getBindingLocation() == null) {
				return "<AND><Presence Path=\"" + definitionIF.getConstraintPath() + ".1[1]" + "\"/><PlainText Path=\""
						+ definitionIF.getConstraintPath() + ".1[1]" + "\" Text=\"" + ifData.getValueData().getValue()
						+ "\" IgnoreCase=\"false\"/></AND>";
			} else {
				// "1 or 4", "1 or 4 or 10"
				if (ifData.getValueData().getBindingLocation().equals("1 or 4")) {
					...
					return "<OR><OR><AND><Presence Path=\"" + definitionIF.getConstraintPath() + ".1[1]"
							...
							+ "\"/><PlainText Path=\"" + definitionIF.getConstraintPath() + ".10[1]" + "\" Text=\""
							+ ifData.getValueData().getValue() + "\" IgnoreCase=\"false\"/></AND></OR>";
				} else {
	...
	private boolean isValidData(CoConstraintTHENColumnData thenData, CoConstraintColumnDefinition definitionThen,
