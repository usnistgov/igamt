...
public class SerializableDatatype extends SerializableSection {
	...
	private String host;
	private IncludeAutoGenerated includeDerived;
	...
	public SerializableDatatype(String id, String prefix, String position, String headerLevel, String title,
			Datatype datatype, String defPreText, String defPostText, String usageNote,
			...
			List<ValueSetOrSingleCodeBinding> filtredBinding, IncludeAutoGenerated includeDerived,
			BindingExportConfig bindingConfig) {
		...
		this.setIncludeDerived(includeDerived);
	}

	@Override
	public Element serializeElement() throws DatatypeSerializationException {
		Element datatypeElement = new Element("Datatype");
		if (this.datatype != null) {
			if (this.datatype.isTemporary()) {

				return this.serializeTemporay();
			}
			try {
				...
				datatypeElement.addAttribute(
						new Attribute("Hl7Version", datatype.getHl7Version() == null ? "" : datatype.getHl7Version()));
				datatypeElement.addAttribute(new Attribute("id", datatype.getId()));
				...
				if (this.filtredBinding != null && !this.filtredBinding.isEmpty()) {
					if (this.bindingConfig != null && this.bindingConfig.isIncludeForDatatype()) {
						Element valueSetBindingListElement = super.createValueSetBindingListElement(this.filtredBinding,
								this.tables, datatype.getLabel(), this.bindingConfig);
				...
				if (datatype.getComponents().isEmpty()) {
					datatypeElement.addAttribute(new Attribute("primitive", "true"));
				} else {
					datatypeElement.addAttribute(new Attribute("primitive", "false"));
				}
									...
									if (datatype != null) {
										...
										if (datatype.getComponents().size() == 0) {
											...
											if (component.getMaxLength() != null
													&& !component.getMaxLength().equals(""))
												componentElement.addAttribute(
														new Attribute("MaxLength", component.getMaxLength()));
											if (component.getConfLength() != null
													&& !component.getConfLength().equals(""))
												componentElement.addAttribute(
										...
										throw new DatatypeNotFoundException(component.getDatatype().getId(),
												component.getDatatype().getLabel());
									}
								}
								if (this.componentValueSetBindingsMap.containsKey(component)) {
									List<ValueSetOrSingleCodeBinding> valueSetBindings = this.componentValueSetBindingsMap
											.get(component);
	...
	private Element serializeTemporay() throws DatatypeSerializationException {

		Element datatypeElement = new Element("Datatype");
		if (this.datatype != null && this.includeDerived != null) {
			try {
				Element datatypeMetadata = super.createMetadataElement(datatype);
				if (datatypeMetadata != null) {
					datatypeElement.appendChild(datatypeMetadata);
				...
				if (this.includeDerived.purposeAndUse) {
					datatypeElement.addAttribute(new Attribute("PurposeAndUse", datatype.getPurposeAndUse()));
				}
				datatypeElement.addAttribute(new Attribute("Comment", datatype.getComment()));
				datatypeElement.addAttribute(
				...
				if (this.includeDerived.binding) {
				...
				if (this.includeDerived.comments) {
					if (datatype.getComments() != null && !datatype.getComments().isEmpty()) {
						Element commentListElement = super.createCommentListElement(datatype.getComments(),
								datatype.getLabel());
						if (commentListElement != null) {
				...
				if ((datatype != null && (!this.defPreText.isEmpty()) || !this.defPostText.isEmpty())) {
					if (this.includeDerived.preDef) {
						if (this.defPreText != null && !this.defPreText.isEmpty()) {
							datatypeElement.appendChild(this.createTextElement("DefPreText", this.defPreText));
						}
					}
					if (this.includeDerived.postDef) {
						if (this.defPostText != null && !this.defPostText.isEmpty()) {
							datatypeElement.appendChild(this.createTextElement("DefPostText", this.defPostText));
						}
					}
				}
				if (this.includeDerived.usageNotes) {
					if (this.usageNote != null && !this.usageNote.trim().isEmpty()) {
						datatypeElement.appendChild(this.createTextElement("UsageNote", this.usageNote));
					}
				}
	...
	public IncludeAutoGenerated getIncludeDerived() {
		return includeDerived;
	}

	public void setIncludeDerived(IncludeAutoGenerated includeDerived) {
